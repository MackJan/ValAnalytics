...
const Sidebar: React.FC = () => {
    ...
    return (
        <div className={`bg-white border-r border-gray-200 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-68'} h-screen flex flex-col shadow-lg`}>
            {/* Header */}
            <div className="p-4 border-b border-gray-200 flex-shrink-0">
                <div className="flex items-center justify-between h-10">
                    {!isCollapsed && (
                        <h2 className="text-xl font-bold text-gray-800 whitespace-nowrap overflow-hidden">Performance Tracker</h2>
                    )}
                    <button
                ...
                <nav className="space-y-2 px-3">
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <Icon size={20} className={`${active ? 'text-blue-600' : 'text-gray-500'} group-hover:text-gray-700 flex-shrink-0`} />
                                {!isCollapsed && (
            ...
            <div className="p-4 border-t border-gray-200 flex-shrink-0">
                <button className={`w-full flex items-center px-3 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors min-h-[44px] ${
                    isCollapsed ? 'justify-center' : 'space-x-3'
                }`}>
