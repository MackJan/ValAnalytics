...
def post_user_and_match():
    ...
    for match_data in matches:
        ...
        team_blue_response = json.loads(requests.post(f"{base_url}/matches/{match_id}/teams/", json=team_blue, headers=headers).text)
        print(team_blue_response)
        team_red_response = json.loads(requests.post(f"{base_url}/matches/{match_id}/teams/", json=team_red, headers=headers).text)
        ...
        for player_data in data["players"]:
            ...
            player = {
                ...
                "agent": json.loads(requests.get(f"https://valorant-api.com/v1/agents/{player_data["agent"]}").text)["data"]["displayName"]
            }
